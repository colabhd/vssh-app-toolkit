...
| **1.3** | Perfil `headless`: campo em `servers`, `--headless` no provisionamento, **portal serve o shell** | médio |
...
`servers` é `SCHEMALESS`, então é só um campo: `profile: 'x11' | 'headless'` (default `'x11'`).
`provision-base.sh` ganha `--headless`, que pula a instalação do xpra, openbox, `/etc/xpra/conf.d`,
`Xwrapper.config`, `/run/xpra` e o timer de atualização do cliente. **Mantém**:
`vssh-app-install`/`-run`/`-supervisor`, `inotify-tools`, `python3`, `vssh-setup-user`.
