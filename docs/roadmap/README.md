...
| 10 | [Motor X11](09-motor-x11.md) — item 2: a ponte WS → socket nativo, e a última porta morre | `vsshapp-xpra` | 📋 planejado · `--bind-ws` aceita só `HOST:PORT` (medido), então a ponte desembrulha o WebSocket e escreve no `--bind` unix, que aceita `connect()` e fica de pé. **Metade da dúvida caiu lendo o cliente**: os bytes dentro dos frames binários SÃO o protocolo nativo (`Protocol.js:497-525` — cabeçalho `'P'` + flags + tamanho BE), e o receptor remonta atravessando fronteira de pedaço (`:265-296`), então ele nunca supôs frame == pacote. Falta só o lado do servidor. **Enquanto isto não fechar, o `vssh-sso` não pode apagar a orquestração de porta** — ela sobrevive para servir um app só |
