...
| 10 | [Motor X11](09-motor-x11.md) — item 3: **o xpra vira só mais uma forma de adicionar janelas** | `vsshapp-xpra` + vssh-sso | 📋 planejado · **decisão de produto tomada**: o gerenciamento de janelas já existe no ambiente, então a janela X11 não ganha cromo nem marca própria — é uma `VsshWindow` como outra qualquer, e o motor deixa de ser gerenciador para ser produtor. **A medida mostrou que a conta é maior que os proxies**: o ambiente fala DOIS dialetos de "janela" e mantém os dois à mão — **2 adaptadores** de tiling (`xpraAdapter`/`pseudoAdapter`, 60 linhas dizendo a mesma coisa duas vezes), **31 pontos** em 4 arquivos do shell perguntando de que família a janela é (`win.minimized ?? win._minimized`, `win.wid \|\| win._id`…), e **43** ocorrências de `proxy` (27 em código — a nota dizia 47). O proxy é consequência: a janela X11 mora dentro do `#screen`, e sair de lá é o que o apaga. Até os SVGs dos botões são cópia (`Window.js:208-213` ≡ `VsshWindow.js:18-21`) |
| 10 | [Motor X11](09-motor-x11.md) — item 5: **a orquestração de porta morre** (era o 0d da Onda 9) | vssh-sso | 📋 **liberado pelo item 2** · mudou de onda porque **é aqui que ela deixa de ter assunto**: enquanto o xpra declarava `tcp`, os onze lugares, o `nextLoopback` e o teto de 254 servidores existiam para servir **um app só**. A metade que não dependia disso já saiu com o 0c — a porta do túnel é local, e o portal parou de perguntá-la ao servidor. O que resta é ordem de entrega, não técnica: apagar antes de o `vsshapp-xpra` 0.5.0 estar instalado nos servidores deixa o motor sem endereço |
| 10 | [Motor X11](09-motor-x11.md) — item 4: o último jQuery do ambiente sai | `vsshapp-xpra` | 📋 planejado · a Onda 8 tirou o jQuery do shell e o ambiente **continua entregando 30 mil linhas dele** dentro deste app: `jquery-ui.js` (19.061) + `jquery.js` (10.716) + `slick.js` (3.011), para **35 call sites** de `$(`. ~~"a maioria mora no `Window.js`, que o item 3 apaga"~~ — **contado, e é o contrário**: `Window.js` tem 5, `Notifications.js` 9 e `Client.js` **21**. Mas 3 e 4 continuam sendo um trabalho só, **por uma razão melhor**: o maior consumidor de jQuery do `Client.js` é o carrossel Alt+Tab, e ele só existe porque as janelas X11 não estão em lugar que o ambiente enxergue. Feito o item 3, ele não é jQuery a portar — é **código a apagar**, com `slick.js` junto. E já há morto medido: `#float_menu`, o dock do cliente xpra, **não existe** no `index.html` do shell |
