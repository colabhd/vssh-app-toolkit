...
| 9 | [Editor do ambiente](08-editor-do-ambiente.md) — item 2: o workbench é nosso, sobre um fork | `vsshapp-vscode` | 📋 planejado · o VS Code web tem API de embedder pública (`web.api.ts`): `workspaceProvider`, `productConfiguration`, `commands.executeCommand` de qualquer comando, tema, cofre, prefixo. **E o fork é o único lugar que alcança** a resolução de plataforma (`extensionGalleryService.ts:35`), o renderizador do menu de contexto e o seletor de arquivo — os três são constante de módulo ou `registerSingleton`. ⚠ **E medindo o build pronto, a frase "o backend serve a NOSSA página" não se sustenta**: o alvo `reh-web` não produz `workbench.web.main.js`; produz **um** bundle de **33,8 MiB** cujo ponto de entrada **é o embedder de referência**, que lê a `<meta>` e chama `create()` sozinho. `create` só sai de `workbench.web.main.internal.js`, que o alvo empacota e não expõe. O item muda de forma, não de objetivo: o que é **dado** (galeria, tema, layout, indicador) vai no `data-settings` da `<meta>`, reescrito pelo nosso backend **sem recompilar**; o que é **função** (`workspaceProvider`, cofre, `resolveExternalUri`, handlers de comando) vira patch em `workbench.ts:603-628`, a única costura. E o prefixo custa **um header**: o `workbench.html` emite as seis URLs de uma variável só, derivada de `x-forwarded-prefix` (`webClientServer.ts:268,341`). **Essa metade já está de pé** (item 2c, `vssh-sso`): o portal manda `X-Forwarded-Prefix: /<serverId>/proxy/app/<id>` para **todo** app — o header é padrão de fato e um `if` por id seria o começo de N caminhos —, e é uma **função**, não um template na rota, porque o invariante não é o formato e sim que **o pedaço cortado e o anunciado sejam o mesmo**. ⚠ **A primeira rodada de refutação não valia**: o `git checkout` restaurava dois caminhos e falhava inteiro por causa do teste ainda não versionado, então as mutações **se acumulavam** e todo caso ficava vermelho por um motivo alheio — o roteiro agora **confere o restauro** e aborta se ele não aconteceu. **A primeira abertura real provou o prefixo e cobrou o resto**: a página carregou inteira (assets todos sob `/ipprivm01/proxy/app/vscode/oss-<commit>/static/…`), o app bindou o socket e o motor bindou o dele **200 ms depois**, sem uma porta TCP nas duas pontas — **e o WebSocket morreu na CONSTRUÇÃO**, `wss://[vssh.colabh.org:443,80]:443/…`, cinco tentativas até "permanent error", editor sem extension host. `xfwd: true` **acrescenta** ao que já veio (`web-incoming.js:82-83`) e antes do portal passou o Cloudflare: `x-forwarded-port: 443,80`. Uma cadeia não é errada — errado é entregá-la a quem precisa de **um** endereço, e quem o sabe é o portal. A metade que não se vê são as **chaves em minúscula**: `X-Forwarded-Port` não substitui `x-forwarded-port`, cria uma segunda e o cliente manda as duas. ⚠ **E o `getFirstHeader` do upstream** (`webClientServer.ts:262-265`) **só desempacota array** — com a string separada por vírgula ele devolve a lista inteira, um "first" que não é o primeiro; fica registrado, não consertado, porque o portão no portal serve todo app e não custa rebuild. Guarda `prefixo-do-app.test.js`, **12 casos, refutação 12/12** — o caso central monta o `remoteAuthority` como o motor monta e exige que o `URL` do navegador **aceite**. **✅ E o editor subiu**: `ManagementConnection` em **267 ms**, `ExtensionHost` em **209 ms**, `Launched Extension Host Process`, `remoteAuthority=vssh.colabh.org:443` — e o **`File > Open Folder` mostrou a home**, que é a prova de ponta a ponta (filesystem remoto ⇒ extension host vivo ⇒ WS atravessou portal + os dois sockets unix). **A hipótese das quatro extensões se confirmou**: elas rodam no host **remoto**, não faltava o pacote de navegador — faltava o host; nenhum rebuild do motor foi preciso. O resto do console é ruído catalogado (`vsda` é proprietário e não existe no OSS; `agent-host-bridge` não é configurado de propósito; `*.js.map` é o build sem minificação) |
...
| 9 | [Editor do ambiente](08-editor-do-ambiente.md) — item 2d: a barra do editor É a barra da janela | toolkit + vssh-sso + `vsshapp-vscode` | ✅ **concluído** (`vscode` **0.1.26**, shell **4.3.0**, toolkit **4.2.0**) · o manifesto ganha `window.cabecalho: "app"`, o shell para de desenhar o cabeçalho dele, e os três botões entram no `.window-controls-container` que o `titlebarPart.ts` já constrói. ⚠ **O que estava escrito aqui invertia o item**: dizia `customTitleBarVisibility: 'never'` — "a barra do VS Code **sai**" —, e quem sai é a **nossa**; o valor é `'auto'`. **Arrastar não custou geometria nenhuma**: a região de arraste é *prependada* (`:486`) e tudo mais é *appendado* por cima (`:550`), então um botão recebe o próprio clique por estar acima, não por alguém tê-lo excluído — o oposto do retângulo invisível sincronizado a cada layout do `/proxy/vscode/`. **E a janela real cobrou três defeitos que nenhuma bancada via.** (1) Os botões não apareciam **e estavam no DOM**: o container do upstream vale `width: 0px` na web (`titlebarpart.css:308,325`) e um item de flex com o `flex-shrink: 1` padrão encolhe até zero por mais que declare 46px — medido, container `0×35`, três botões medindo zero. (2) O arraste ficou **"meio toggle"**, e a causa estava escrita **como acerto** no meu próprio comentário: "mesmo limiar" e "a `guard` já cobre o iframe" — o `iniciar()` só roda **depois** do limiar, e o limiar precisa de um `pointermove` que não pode atravessar o iframe; sem `guard`, nem o move nem o `pointerup` chegam. **O limiar é de quem tem o ponteiro.** (3) A janela ficou **sem controle nenhum** com o motor certo: os dois blocos são guardados por `if (ponte?.window?.arrastar)`, e o `package-lock.json` do app fixava o toolkit **4.0.0**. A lição "mover a tag `v4`" estava **incompleta** — a tag moveu; quem a ignora é o `npm ci`. **Um guarda opcional que dá falso não quebra: ele desliga o recurso**, calado. Guardas: **9 casos** (`cabecalho-do-app.test.js`) + **4** (`pagina-do-ambiente.test.js`), refutação **9/9 + 6/6**. ⚠ E um caso ficou **verde o tempo todo com a janela quebrada**: montava com `distance: 0`, e com limiar zero a `guard` sobe de qualquer jeito |
| 9 | [Editor do ambiente](08-editor-do-ambiente.md) — item 2e: o gesto do arraste, o banner e o cache dos estáticos | toolkit + vssh-sso + `vsshapp-vscode` | ✅ **concluído** (`vscode` **0.1.27**, shell **4.4.0**, toolkit **4.3.0**) · **o arraste não era um detalhe errado, era o DESENHO**: o app dizia "me agarraram aqui" e o ambiente assumia o resto pela `guard` — uma entrega no **meio** do gesto, que depende de a `guard` subir antes do próximo evento **e** de ganhar o empilhamento. Dois consertos meus só trocaram o sintoma, e o critério mudou: em vez de mais um conserto pontual, **um desenho em que a falha não seja possível**. Quem tem o ponteiro é o app, e com `setPointerCapture` ele tem o gesto inteiro — fora do próprio quadro e inclusive o `pointerup` —, então reporta `arrastar` / `arrastarPara` / `arrastarFim`. **Descartado com motivo:** tirar o cabeçalho do iframe resolveria e custa a barra do VS Code inteira (menu, command center, título) reconstruída no shell — a mesma inversão que arrancamos do `/proxy/vscode/`, apontada para o outro lado — e não remove a fronteira, só a desce 35 px. **O banner** do Workspace Trust ocupava o topo da janela: `shouldShowBannerFirst()` (`layout.ts:1385-1387`) põe o banner acima da barra na web, e o upstream está certo para uma aba — a premissa é que não existe barra de janela. Metade **dado** (`security.workspace.trust.enabled: false`, que não protege nada num editor com terminal integrado) e metade **função** (patch 0008, que vale para o banner de amanhã). **E o `Shift+F5` tinha outra causa, medida:** `getServerProductSegment` devolve `${quality ?? "oss"}-${commit ?? "dev"}` (`network.ts:256-258`), e os estáticos sob ele vão com `max-age=31536000` (`webClientServer.ts:69`) — **um ano**. Todo motor nosso nasce do mesmo commit do upstream: mesma URL, marcada imutável, bytes diferentes. O navegador obedecia. Cada motor carimba o próprio commit — e o carimbador virou script único porque a reescrita do `product.json` estava no `motor.yml` **e** no `build-local.sh`, a mesma duplicação que custou um build. Guardas: **10 casos** + **7**, refutação **13/13**. ⚠ **Duas guardas minhas mediram a coisa errada:** uma ficou verde com a janela quebrada (montava com `distance: 0`, onde o limiar não existe) e a outra vermelha sem defeito (perguntava pelo MECANISMO, `doc.addEventListener("pointermove")`, e reprovou quando ele melhorou). Terceira vez do mesmo erro: **a pergunta é o invariante, nunca a implementação dele** |
| 9 | [Editor do ambiente](08-editor-do-ambiente.md) — itens 2f/2g: eu publiquei um editor que recusava a própria conexão | `vsshapp-vscode` + vssh-sso | ✅ **concluído** · **o conserto do `Shift+F5` quebrou o editor, e o defeito ficou publicado.** Carimbei o `commit` no `product.json` DEPOIS do build — mas ele é embutido no bundle do **cliente** em tempo de compilação (`gulpfile.reh.ts:41,420`), e o servidor compara os dois no handshake (`remoteExtensionHostAgentServer.ts:335-341`): `Client refused: version mismatch`. **Carimbar depois do build carimba um lado só, e os dois lados se conferem.** O carimbo foi para `BUILD_SOURCEVERSION`, antes de compilar — e o formato não é escolha: `getVersion` só o aceita casando `/^[0-9a-f]{40}$/i`, fora disso **ignora em silêncio**. **O que fica é o portão:** o carimbador abre o bundle do cliente e exige achar lá o commit que o servidor vai anunciar — a pergunta que nenhuma bancada podia fazer, porque ela só existe depois do build. **O tremor do arraste** era a mesma família: coordenada de dentro do iframe é um laço de realimentação, porque o quadro do app se move JUNTO com a janela e `clientX` vale `tela − posição da janela`; com mais de um `pointermove` por quadro, o ambiente reconverte contra um layout de um quadro atrás. Os pontos do meio vão em `screenX`/`screenY`, com o deslocamento fixado UMA vez. **E o Copilot cobrou o terceiro build:** o script dele liga o modo de pipeline oficial ao ver `BUILD_SOURCEVERSION` e passa a exigir `VSCODE_QUALITY`, depois `VSCODE_PUBLISH_COUNTER` — um por build derrubado, 5,3 min cada. Alimentei duas variáveis antes de ver o óbvio: **construir o que se descarta era alimentar a colisão para sempre**. A dívida estava registrada desde o item 1 e agora tem preço; de brinde, **8,03 → 5,08 min**. Guardas: **38 casos** + **12**, refutação **7/7 + 4/4** |
| 9 | [Editor do ambiente](08-editor-do-ambiente.md) — item 2h: o arraste ficou certo e a janela parou de encaixar nas bordas | vssh-sso | ✅ **concluído** (shell **4.4.1**, sem rebuild do motor e sem reinstalação) · o último da série, e o mais curto: ao separar a conta do movimento do evento que a traz eu escrevi `if (e) opts.drag?.(e)`. Um ponto que vem de fora **não traz evento**, então o callback não acontecia — e quem escuta ele é o `TilingManager.onDrag`. Sem ele não há `_pendingZone`; sem `_pendingZone` o `onDragStop` não tem o que aplicar. **Levei o ponto e deixei o evento para trás**, e o sintoma foi um arraste perfeito que não encaixava em borda nenhuma. O consumidor lê `pageX ?? clientX` e mais nada (`TilingManager.js:419-420`), então o ponto basta: o evento sintético não é remendo, é dizer a mesma coisa para quem só perguntava isso — e o gesto que nasce no shell continua entregando o evento **original**. Guarda: **13 casos**, refutação **3/3**, incluindo a mutação simétrica (trocar o evento real por um sintético e perder campos que ninguém pediu para perder) |
| 9 | [Editor do ambiente](08-editor-do-ambiente.md) — item 2i: **VSSHCode**, o nome e um ícone que existe | `vsshapp-vscode` | ✅ **concluído** (`vscode` **0.1.28**) · o nome vivia em **dois lugares dizendo coisas diferentes** — o manifesto dizia `Editor` (Launchpad, barra de tarefas, alternador) e o `product.json` dizia `VSSH Code` (barra de título do editor) —, e com `cabecalho: "app"` os dois aparecem na **mesma janela**, um dentro do outro. **E o ícone anterior não era só sem graça: estava invisível.** Monolinha em `currentColor`, e o shell desenha ícone de app com `<img src>` (`Launchpad.js:203-208`) — dentro de um `<img>` o SVG é documento independente, `currentColor` não herda e cai para **preto**, sobre um desktop escuro, com o arquivo "correto" e nada acusando. O desenho novo é o **irmão do favicon do portal** (`>_` monoespaçado em `#0c0c0e`+`#0e639c`): a última palavra muda, e o `_` de shell vira o **cursor de bloco** no verde que o tema chama de *cursor terminal*. Escolhido entre três, **julgados renderizados de 16 a 64px** no chão do desktop, no Launchpad e na barra de tarefas — as descartadas eram uma janela com faixa de cabeçalho (bonita a 64, borrão a 24) e um bloco de código (a mais legível e a **menos nossa**). ⚠ **O que não acompanha o rótulo:** `serverApplicationName`, `applicationName`, `dataFolderName` e o `id` — **identificador não é nome de exibição**; o build procura `bin/vssh-code-server` pelo nome e o `id` vira caminho, porta e chave no repositório. Guarda: **40 casos**, refutação **5/5** — e a do ícone nasceu vermelha lendo o **comentário** que explica por que `currentColor` não é usado, a mesma armadilha do `soCodigo` |
| 9 | [Editor do ambiente](08-editor-do-ambiente.md) — item 2j: a marca se troca no EMPACOTAMENTO | `vsshapp-vscode` | ✅ **concluído** (`vscode` **0.1.30**) · o rename parou na porta: a página do motor aponta para `resources/server/favicon.ico`, `code-192.png` e um `manifest.json` que é **arquivo estático** — e por isso ele continuava dizendo `"name": "Code - OSS"` depois de o `product.json` virar VSSHCode. Um **terceiro** lugar guardando a identidade, e o único que ninguém tinha atualizado. ⚠ **A primeira versão trocava os três `<link>` reescrevendo a página em runtime, e o lugar estava errado**: funciona, e falha **calada** — se o upstream mexer na marcação, a expressão não casa, o logo do VS Code volta e ninguém sabe. Foi o usuário que apontou o certo (*"a gente não pode copiar os arquivos estáticos para o tarball?"*): no empacotamento, junto da galeria e do commit, onde a mesma cirurgia **aborta**. E não custa rebuild, porque vestir o produto é passo pós-build. O webmanifest passa a ser gerado do `product.json` — mesma fonte do nome que a barra de título usa. **O que não dá:** gerar os PNG e o `.ico`; rasterizar um SVG pede um renderizador, e trazê-lo para o build por três arquivos seria pior que o problema — eles ficam no tarball sem ninguém apontando para eles |
| 9 | [Editor do ambiente](08-editor-do-ambiente.md) — item 2k: o `window-appicon`, a marca d’água, e o portão para o que eu não vi | `vsshapp-vscode` | ✅ **concluído** (`vscode` **0.1.31**) · o 2j trocou um diretório e eu dei o assunto por encerrado. **O usuário achou o resto colando a URL**: `out/media/code-icon.svg` — e ela dói mais que o favicon, porque o CSS a desenha em **`.window-appicon`**, à esquerda da barra de título, na mesma faixa dos nossos botões. **Terceira vez do mesmo defeito nesta onda** (uma das duas chamadas de `timeout` no CI; `resources/server` sem `out/media`), então além do arquivo entrou um **portão para o lugar que eu não conheço**: `#167abf` é o azul da marca do VS Code e a varredura mediu que ele não existe em mais nenhum SVG do artefato — qualquer arquivo que saia com ele **derruba o build**. **A marca d’água virou nossa** — e ⚠ corrigindo uma leitura: o `letterpress` do upstream **não é o logo deles**, é uma ilustração de editor, preta a 30%, sem filtro no CSS; trocá-la não é tirar marca alheia, é ter a nossa. A versão VSSHCode é o **símbolo** (chevron e cursor, sem a placa), porque marca d’água quer símbolo: a placa viraria um retângulo no meio do editor. Uma geometria, quatro variantes geradas dela. ⚠ **E o molde é um SVG DE VERDADE** — a pergunta foi do usuário (*"stroke=`__COR__`?"*) e pegou uma contradição de uma linha: eu tinha acabado de escrever uma guarda dizendo que ícone tem de **abrir** e criei um `.svg` que não abre. ⚠ **O passo virou idempotente** depois de o segundo `empacotar` abortar: cobra-se o **estado final**, não a transição — terceira aparição da mesma lição. Guarda: **42 casos**, e o da iconografia executa o carimbador **seis vezes** |
...
| 9 | [Editor do ambiente](08-editor-do-ambiente.md) — item 5: a extensão VSSH servida pelo próprio app | `vsshapp-vscode` + toolkit | ✅ **concluído** · uma feature de **duas metades, e nenhuma serve sozinha**: `asMenuId` (`web.factory.ts:76-81`) conhece **dois** MenuIds, então `explorer/context` não é alcançável de fora; e uma extensão web roda num **Web Worker**, sem `window` nem `parent`, então a ponte `vssh.*` é inalcançável de lá. O elo é o comando — `mainThreadCommands.ts:92` cai no mesmo `CommandsRegistry` em que `web.factory.ts:45` escreve. E `extHostExtensionService.ts:87` carrega a extensão com `new Function(...)`: **CommonJS puro, sem bundler**. ⚠ **A VOLTA não existia, e três bancadas verdes não viram**: nada no app escutava `open-context`, então a janela abria e o caminho ia para lugar nenhum — as três mediam a IDA. Conserto no patch `0010`, com `tipo` separando arquivo de pasta (acréscimo compatível). ⚠ **E `forceReuseWindow` estava errado**: o ambiente focou aquela janela por ser a do APP, não por ser o lugar certo para AQUELA pasta — vai `forceNewWindow`, e ele não é opcional (`vscode.openFolder` defaulta a reusar). ⚠ **Quatro defeitos do `build-local.sh`, todos da mesma família — a árvore do container é ESTADO e o script a tratava como cache**: ele vinha aplicando **3 dos 9 patches** dizendo "conferidos" (o `docker cp` aninhava o diretório), o `0007` carregava uma cópia do hunk do `0003`, a contagem contava tentativas, e `compilar` compilou uma árvore defasada — **quase publiquei o build sem o conserto pela segunda vez**. Guarda `extensao-do-ambiente.test.js`, **21 casos, refutação 14/14** (+6/6 no `open-context`) |
| 9 | [Editor do ambiente](08-editor-do-ambiente.md) — item 5a: **o `chrome.js` contra o app de pé** | toolkit + `vsshapp-vscode` | 📋 **item novo, e ele nasceu de uma crítica ao formato das guardas** · *"um teste que só testa se a cura pra uma doença já conhecida ainda está lá é só perda de tempo de CI"* — está certo, e esta onda tem a prova: as guardas de texto ficaram vermelhas **três vezes lendo os próprios comentários**, e nenhuma pegou o buraco do `open-context`. Saíram **oito `grep`** na fonte do upstream; a pergunta legítima deles foi para `motor-nova-versao.yml` (aplica os patches **de verdade** na tag nova, abre PR ou issue — pronto e **desligado**, à espera de runner self-hosted). Entrou o **smoke de serviço**: sobe o app inteiro contra o motor recém-construído e pergunta, com o melhor caso baixando o **bundle servido** e cobrando as pontes lá dentro. **Mas executar prova COMPORTAMENTO, não ALCANCE** — o `open-context` era falta de alcance, e não havia bloco para executar. O único instrumento que pega isso é o navegador contra o app de pé, e o toolkit já o tem (`tests/browser/chrome.js`, CDP à mão). **O que o derruba:** o custo de manter um Chrome dirigido passar do que ele pega — a resposta vem da primeira vez que ele achar o que o smoke não achou |
...
| 9 | [Editor do ambiente](08-editor-do-ambiente.md) — item 8: **o fim de vida de um vssh-app** | toolkit + vssh-sso | 📋 planejado · **é de plataforma; o editor só foi quem revelou.** Fechar a janela de um app **não encerra nada**: `POST /api/apps/:id/stop` existe (`apps.ts:371`) e **ninguém o chama** — varredura no `vssh-client/`, zero invocações a partir do fechamento. A conta não é por app, é por **(usuário × app já aberto uma vez)**: quem abriu em março ocupa RAM em agosto, sem janela na tela. ⚠ **E o número ainda não existe** — os 9,3 GiB medidos são de uma **sessão inteira**, não do motor ocioso; medir o RSS sem cliente conectado é o primeiro passo do item, e teto escrito antes disso é decoração (foi o que já aconteceu com o `resources` do manifesto). **E "fechou, morreu" está ERRADO para os apps que mais custam**: o log da primeira sessão real traz `VSCODE_RECONNECTION_GRACE_TIME=10800000ms` — **3 h** —, porque o servidor do VS Code é feito para sobreviver ao navegador (é assim que o terminal integrado continua vivo); e o incidente de 11/08 registrou como **ganho** a sessão X11 sobreviver ao backend. Então o padrão de hoje ("nunca encerra") foi escolhido **por omissão**. Entra `backend.aoFechar`: `encerrar` (padrão), `manter`, `ocioso:<N>m` — e **não** cabe no `kind`, que governa start e supervisão, não fim de vida. Para o `vscode` o valor é `ocioso`. O pedaço difícil é "sem processo filho ativo": um motor de editor sempre tem filhos, e sem boa resposta ali o `ocioso` vira temporizador que mata trabalho |
...
| 10 | [Motor X11](09-motor-x11.md) — item 3: **o xpra vira só mais uma forma de adicionar janelas** | `vsshapp-xpra` + vssh-sso | ✅ **3a e 3b concluídos** · 📋 **3d** (o pacote) · o gerenciamento de janelas já existe no ambiente, então a janela X11 é uma `VsshWindow` como outra qualquer e o motor deixa de ser gerenciador para ser **produtor**. **O 3b apagou o que a medida tinha achado**: os **dois** dialetos de janela viraram um (`adaptadorDeJanela`), o observador de escala do `index.html` saiu (ele já tinha perdido METADE do assunto quando o pacote 0.7.x tirou o `transform` do `#screen`), e as duas máquinas de espelho — uma por janela, outra por ÍCONE da área de trabalho — viraram **uma linha de CSS**: `#screen { pointer-events: none }` com `#screen > * { auto }`, porque as superfícies NUAS do motor continuam morando lá. **326 linhas entram, 560 saem.** Dois defeitos que só a medida mostrou: `allTiled()` devolvia a mesma janela X11 **duas vezes** (ela está nos dois registries, e é o mesmo objeto), e o menu de contexto da barra chamava um verbo e o botão do cromo outro — numa janela do shell, aquele menu não fazia **nada**, em silêncio, por causa de um `?.`. Guarda `arraste-entre-janelas.test.js`, **7 casos num Chrome de verdade** (a pergunta é hit-test, e ela mede **alcance**, não comportamento), **refutação 4/4**. ⚠ E o `screen-scale-proxies.test.js`, que existia para proteger a mecânica que saiu, foi **invertido e depois apagado no mesmo dia**: quatro dos cinco casos perguntavam se um símbolo continuava ausente, que é o que a [regra dos testes](#a-regra-dos-testes-decidida-em-2026-08-12-e-vale-daqui-para-frente) proíbe. Sem perda de cobertura — a lição é que guarda de mecânica removida não INVERTE, é substituída por uma que **execute** o comportamento que a mecânica servia |
| 10 | [Motor X11](09-motor-x11.md) — item 3d: **o pacote atende pelo verbo do ambiente** | `vsshapp-xpra` | 📋 **o resto do 3b, e ele não cabia no `vssh-sso`** · a ponte entre os dois dialetos está feita pela metade: `XpraWindow` sobrescreve `toggleMinimized()` (sem ele o app se acha visível), mas **não** `toggleMaximized()` nem `togglePinned()` — existem só `toggle_maximized`/`toggle_pinned`, que chamam o verbo do ambiente e depois fazem o que é do X11. Como o botão do cromo chama o verbo do ambiente, **maximizar uma janela X11 pelo botão nunca contou o tamanho novo ao X11** — não é regressão do 3b, é o estado de hoje, que ele tornou visível ao unificar o caminho. Vai junto o `MenuCustom.js`, que fala `window.client.focused_wid` e `client.set_focus` direto: o shell nomeando o motor, que a 2.7 proíbe. Exige publicar o pacote |
...
| 10 | [Motor X11](09-motor-x11.md) — item 8: **as pastas do usuário** | vssh-sso | ✅ **feito, e ele veio da conferência do 3b numa sessão real** · *"arrastar do gerenciador para a área de trabalho não faz nada"* — e não era regressão do 3b: a home da conta não tinha `Desktop`, então `GET /api/desktop-files` respondia `desktopPath: null` e **todo** gesto da área de trabalho saía calado (`if (!_desktopPath) return`). O ambiente pressupunha a pasta e ninguém a criava. O conserto é de onde ela NASCE, não de quem a procura: o `vssh-setup-user.sh` a cria pelo mesmo mecanismo que a rota já consulta, e como ele roda a cada provisionamento (não só no `useradd`), quem já tem conta ganha a pasta na próxima sessão. O caminho é LIDO de volta e não escrito à mão — o XDG respeita o locale, e presumir o nome reintroduziria o palpite que a rota evita. ⚠ **E uma frase deste item estava errada, valendo o item inteiro**: eu escrevi que o script roda a cada provisionamento. Não roda — os dois chamadores o invocam dentro de `if (isNewUser)`, uma vez, no `useradd`, e do jeito que estava o conserto não alcançaria **ninguém que já usa o ambiente**. A criação foi também para onde a pergunta é feita toda vez (o próprio `GET /api/desktop-files`), com a precedência declarada nos dois arquivos. Guarda `pastas-do-usuario.test.js`, **7 casos, refutação 3/3, todos executando** — os dois lados rodam de verdade, cada um numa HOME de mentira |
| 10 | [Motor X11](09-motor-x11.md) — item 8a: **a barra lateral pergunta em vez de adivinhar** | vssh-sso | ✅ **feito** · ela declarava sete lugares com nome fixo e montava `home + '/' + nome`. A lista **não bate com locale nenhum** — em `C` o XDG cria `Documents`/`Music`, em `pt_BR` `Documentos`/`Música` —, então seis dos sete atalhos podiam apontar para pasta inexistente e a barra os desenhava igual: clicar levava a uma listagem vazia. Agora é `GET /api/user-dirs`, **uma ida de SSH para as seis**, e **lugar sem resposta não é desenhado**. Sem `xdg-user-dirs` instalado a resposta **não volta a ser palpite: vira SONDA** — os nomes conhecidos são testados com `-d`, e **a diferença entre adivinhar e sondar é esse `[ -d ]`**. O script mora num módulo FOLHA porque `routes/system.ts` levanta express, db e SSH quando importado. Guarda **6 casos, refutação 3/3**, executando o script real contra uma HOME de mentira |
| 9 | [Editor do ambiente](08-editor-do-ambiente.md) — item 8: **o fim de vida de um vssh-app** | toolkit + vssh-sso | ✅ **feito** · `POST /api/apps/:id/stop` existia desde sempre e **ninguém o chamava**: fechar a janela tirava o iframe da tela e o backend vivia até a sessão acabar. A conta não era por app — era por **(usuário × app já aberto uma vez)**: um app aberto em março seguia ocupando RAM em agosto, em toda sessão daquela conta. Agora há contrato (`backend.aoFechar`), e o padrão é **`encerrar`** porque **um padrão que vaza memória tem de ser o que se ESCOLHE, não o que se herda**. `kind:"service"` **manda** sobre o campo — um daemon não morre com uma janela —, e essa precedência mora num módulo folha para poder ser executada. ⚠ **"A última janela" não é pergunta que UMA aba saiba responder**: `findWindows` vê só a própria página, e fechar na aba A mataria o backend da janela da aba B. Quem encerra **pergunta às outras abas** por `BroadcastChannel` e só age no silêncio. O `ocioso:<N>m` **não entrou de propósito** — depende de duas medidas que não temos, e um valor aceito que não faz nada é pior que um ausente. Guarda **10 casos**, com o instrumento consertado antes de valer |
| 10 | [Motor X11](09-motor-x11.md) — ⏸ **o pacote deve DUAS versões** | vsshapp-xpra | ⏸ **medido: os servidores estão na 0.7.6**, e o repositório tem 0.7.7 e 0.8.0 commitadas e **não empurradas** — um push publica as duas. A 0.7.7 leva "uma carga por página, e ela retoma"; a **0.8.0 conserta um defeito que está NO AR**: desde que o item 3b subiu ao shell, `Window.js` chama `TilingManager.pseudoAdapter` em cinco pontos do `_setupDragResize`, e `onDragStart(this, undefined)` **lança** em `adapter.tileable` — **arrastar qualquer janela X11 quebra no primeiro movimento**. Publicar não tem rota de retirada, então a decisão é do dono; o que fica registrado é **o preço de esperar** |
| 10 | [Motor X11](09-motor-x11.md) — item 3e: **o pacote registra o próprio host** | vsshapp-xpra + vssh-sso | 📋 **item novo, e é a metade do 3d que não cabia numa inversão de verbo** · o `MenuCustom.js` ainda diz `window.client` em três lugares, contra a regra escrita no topo do `vssh-host.js`. Dois já têm par (`nativeWindow`, `focusedNativeWid`); o terceiro **não**, e não é repasse: `activateNativeWindow` **zera `focused_wid` antes** de `set_focus`, porque `set_focus()` faz early-return quando o wid já está focado — e uma janela do shell focada não zera o foco X11. Sem aquela linha a janela X11 nunca recupera o `.taskbar-active`. ⚠ **Acrescentar a capacidade hoje não a faz funcionar:** o `host-xpra.js` mora no pacote e **ninguém o instala** (`autoSelect()` escolhe o standalone desde a 2.7) — é a mesma razão de `captureKeyboard()` ser no-op. Ligá-lo acende junto o `nativeWindows()`, que **duplica** o que `VsshWindow._all` já entrega, e o `workarea()`, que devolve pixel de DISPOSITIVO onde o shell espera pixel CSS. É ele que faz o ambiente voltar a poder dizer *"este teclado é meu"* |
| 10 | [Motor X11](09-motor-x11.md) — item 5: **o teto de 254 servidores morreu** | vssh-sso | ✅ **feito** · o `nextLoopback` dava a cada servidor um `127.0.0.x` próprio, varrendo de 1 a 254 e devolvendo `127.0.0.1` **calado** quando acabava — o 255º passaria a disputar o bind com o primeiro, e o sintoma seria o túnel de um usuário caindo no de outro. **Ninguém escolheu esse limite: ele caiu do ESPELHAMENTO** (a ponta local do `-L` repetia a porta que o app bindava no servidor). O espelhamento acabou, e quem separa dois servidores é a mesma máquina que já separava dois usuários: dez mil portas e um sondador de bind. O gate era operacional e passou — o último app em `transport: "tcp"` era o xpra, e o manifesto dele diz `socket` desde a 0.5.0, então a justificativa escrita no schema **deixou de valer**. Apagados: `_allocateAppPort` + a varredura `ss -tlnp` remota, `_findRunningAppPort` (`/proc/environ`), `_reconcileAppPort` — **as duas últimas resolviam um problema que só existia porque duas coisas escolhiam a porta e podiam discordar** —, os três ramos `tcp` e o 12º lugar (`VSSH_APP_PORT` no `env` de todo app, com a ponta LOCAL do túnel dentro). Guarda **4 casos com sockets de verdade**, porque a pergunta é sobre bind |
| 10 | [Motor X11](09-motor-x11.md) — item 3d: **o pacote atende pelo verbo do ambiente** | vsshapp-xpra | ✅ **no disco, 0.8.0, NÃO publicado** · a ponte entre os dois dialetos de janela estava feita pela metade — e a medida mostrou que era pior: **a direção que existia não tinha um único chamador**. `toggle_pinned`/`toggle_maximized` eram nomes mortos; quem chama é o verbo do ambiente (botão do cromo, menu da barra, restauração de sessão), e todos pulavam o espelhamento para o X11. Consequência de todo dia: **maximizar uma janela X11 pelo botão nunca contou o tamanho novo ao X11**. ⚠ E a **terceira regressão do 3b** estava aqui, em **seis pontos**: o repositório continuou chamando `TilingManager.pseudoAdapter`/`xpraAdapter`, que o 3b renomeou e apagou. No `Client.js` o Super+setas devolvia `false` calado; no `Window.js` são cinco pontos e o arraste de **toda** janela X11 morria com `TypeError`. **Nada disso era visível do `vssh-sso`** — lá não há uma linha que cite os nomes. ⚠ **Empurrar o repo PUBLICA no `vssh-repo`, e publicar não tem rota de retirada** |
| 10 | [Motor X11](09-motor-x11.md) — item 4d: **o Alt+Tab e o lado a lado passam a ser do ambiente** | vssh-sso | ✅ **feito** · os dois atalhos existiam — **fora** do ambiente: a tecla era presa no `Client.js`, dentro do pacote do motor X11. Sem o motor não havia atalho nenhum, e com ele o Alt+Tab listava `client.id_to_window` — janelas X11, e nada mais. **Ao medir onde ligá-lo apareceu a TERCEIRA regressão do 3b**: o Super+setas continua no pacote e está quebrado, porque chama `TilingManager.pseudoAdapter` e `xpraAdapter` — o 3b apagou o segundo e renomeou o primeiro, e `handleKey` devolve `false` na primeira linha, **sem erro e sem log**. É a única regressão que mora no outro repositório, e por isso a varredura do `vssh-sso` não a viu. O `TilingManager.handleKey` **não precisou de uma linha**: a máquina de estados do encaixe estava escrita há ondas, sem chamador nenhum no shell — faltava a tecla. O gesto: lista **congelada** na primeira tecla, foco só quando o Alt é **solto** (focar a cada `Tab` reordenaria a lista por uso recente debaixo do dedo, e a terceira janela seria inalcançável). ⚠ **Só chega à página em TELA CHEIA** — fora dela a tecla é do sistema operacional, e o que a torna nossa é a Keyboard Lock que o shell já pede; está dito na tela de atalhos. Limpeza junto: `slick.css` (109 linhas) e `menu-skin.css` → `alternador.css`. Guarda **11 casos**, executando o gesto |
| 10 | [Motor X11](09-motor-x11.md) — item 3c: **arraste de arquivo atravessa a fronteira do app** | toolkit + vssh-sso | ✅ **feito**, e **a medida derrubou metade do item** · os caminhos **já viajavam** no `dataTransfer` (o comentário do `FileOps` que diz o contrário fala da decisão de ACEITAR, no `dragover`, onde `getData` é ilegível), e a direção de **entrada não passa pelo shell**: o app é servido na mesma origem, o `drop` cai no documento dele e o `dataTransfer` é legível direto. O **portão de manifesto que o item previa não existe** — não guardaria nada, porque o backend do app já lê qualquer arquivo que o usuário lê. O que faltava era a **saída**: todo alvo de soltura do ambiente abre o portão em `FileOps.getDragState()`, estado de módulo do documento do shell, que um arraste nascido no iframe nunca escreve. Então o app avisa (`arraste`, fases `inicio`/`fim`) e o shell escreve — e o `fim` é obrigatório porque **`dragend` não atravessa documentos**. A terceira direção (janela → janela do mesmo app) caiu de graça, e é o caso que a guarda mede, porque prova que o contrato é **um**. Guardas de **9 + 7 casos**, cada uma prendendo o literal do MIME por extenso: é o único ponto em que constante vale mais que comportamento, porque o outro lado mora noutro repositório e **cada lado fica verde sozinho com o literal errado** |
| 10 | [Motor X11](09-motor-x11.md) — item 9: **um app desinstalado prendia a sessão** | vssh-sso | ✅ **feito**, achado num log de produção · o lock file de restauração é escrito **por janela** e só é apagado por quem a **fecha** — e desinstalar um app não fecha janela nenhuma. A cada entrada da pessoa no ambiente a restauração pedia `POST /start` de um app que não existe mais, o servidor devolvia **500 com a stack inteira**, o usuário levava um diálogo de erro, e o lock continuava lá para repetir tudo no acesso seguinte: **nada nesse laço melhorava sozinho**. A pergunta certa vem **ANTES do start** (`/api/apps` já é buscada no boot e é ela quem sabe o que existe), e não é o tratamento do erro dele. **O que faz o item é o terceiro valor:** o registro devolvia o mesmo `Map` vazio para *"nenhum app instalado"* e para *"não consegui perguntar"* — e como agora se **apaga arquivo** com essa resposta, ler uma falha de rede como desinstalação levaria **a sessão salva inteira, sem volta**. Só `false` apaga; `null` tenta abrir como antes. De quebra, *"não está instalado"* deixou de ser `500` e virou **404 com código**, com a tradução morando junto de quem lança (na rota um `catch (err)` só enxerga `Error`), e o cliente parou de trocar a frase do servidor por **"HTTP 500"**. Guarda **6 casos executando os dois módulos de verdade**; a refutação foi pulada a pedido, o CI é quem roda |
| 10 | [Motor X11](09-motor-x11.md) — **duas regressões do 3b, achadas na tela** | vssh-sso | ✅ **corrigidas, e a forma das duas é a mesma**: o elemento mudou de casa e o CSS continuou falando do endereço anterior. O **laço ficou invisível** (a regra era `#screen .desktop-laco` e o retângulo passou a nascer no `#desktop-icons`: o div era criado, a seleção acontecia, e ele não tinha `position` nem fundo) e **não dava para mover ícone na grade** (`-webkit-user-drag: none` no `.desktop-icon`, que fazia sentido quando o ícone NUNCA era a origem do arraste — com o proxy fora, ela impedia o `dragstart`, sem erro nenhum no console). **Nenhuma das duas é visível a teste de texto**: a função existe, é chamada, a classe está escrita e está no CSS. A segunda foi reproduzida numa bancada de navegador e achada na PRIMEIRA tentativa, com o gesto indo por `Input.dispatchDragEvent` |
| 10 | [Motor X11](09-motor-x11.md) — item 6: **o portal não deixa dois starts do mesmo app em voo** | vssh-sso | ✅ **feito** — a causa primeira do incidente · dois `startApp` para o mesmo par saíram a **49 ms** de distância; o motor subiu duas vezes, os dois xpra colidiram e o app ficou inalcançável por horas. **A chave é `<servidor, usuário, app>`, e não `<usuário, app>`** como este item dizia: o mesmo usuário pode ter o mesmo app em dois servidores. **O cliente já coalescia e não alcançava o caso** — `AppLauncher._inflightStarts` vive numa PÁGINA, e duas abas são chamadores diferentes; trava mora onde o efeito mora. Três decisões que só a implementação cobrou: **é trava e não cache** (um start que falha solta a chave — guardá-la impediria o par de subir até o pod reiniciar, pior que o defeito original); **o veredito é um e os interessados são N** (descartar o `aoResolver` do segundo deixa a janela dele em "Iniciando…" para sempre); e **`restartApp` NÃO coalesce** — ele espera, senão o `_killAppTree` mata o processo que o start acabou de subir e o start ainda reporta *pronto*. Guarda `um-start-por-vez.test.js`, **5 casos, refutação 5/5**, e ela **executa o `startApp` real**: sem banco ele falha em ~1 ms DEPOIS da trava, e a **identidade do objeto `Error`** diz se dois chamadores compartilharam o trabalho — foi assim que o defeito foi confirmado antes do conserto |
...
| 10 | [Motor X11](09-motor-x11.md) — item 3c: **um vssh-app recebe e produz arquivo arrastado** | toolkit + vssh-sso | 📋 **destravado — o 3b saiu e o `drop` já chega**, medido num Chrome de verdade · pedido a partir do VSSHCode, e a medida mudou a razão · a primeira leitura foi "o proxy de janelas atrapalha"; o bloqueio é **uma linha** — `_acceptsDragRaise() { return false; }` (`VsshWindow.js:176`), sobrescrita só pelo `FileBrowserWindow` (`:1863`). Uma janela de vssh-app **nunca é elevada** durante um arraste, então o proxy invisível fica na frente e o `drop` não chega ao iframe. Não é defeito: é política escrita quando app nenhum podia aceitar arquivo. **Trava no 3b mesmo assim** — construir antes é construir contra a máquina que ele demole (`_dragRaised`, o watchdog de 600 ms, os 43 proxies); com `#screen { pointer-events: none }` o `drop` simplesmente chega. O contrato é **um só**: o MIME `application/x-vssh-files` (`FileOps.js:117`) vira contrato publicado, o app **declara** que aceita no manifesto, **lê** por `vssh.onArquivosSoltos` e **escreve** o mesmo MIME no `dragstart` dele. A terceira direção — entre duas janelas do mesmo app — **cai de graça** das outras duas, e é o teste de que é um contrato e não três integrações |
...
### A regra dos testes (decidida em 2026-08-12, e vale daqui para frente)

**Um teste mede COMPORTAMENTO, executando.** Duas espécies deixam de ser escritas:

| não se escreve | por quê |
|---|---|
| teste que confere se a **correção de um defeito ainda está no código** | ninguém vai tirá-la de lá. É tempo de CI para confirmar que uma linha continua escrita — o CI fica mais caro sem ficar mais informativo |
| teste que confere se algo **removido de propósito continua removido** | ninguém vai reintroduzir o que foi apagado com motivo. E se reintroduzir, é porque decidiu; a guarda só transforma uma decisão em atrito |

A razão de fundo é a mesma nas duas: **um teste de conteúdo pode ir verde sem validar nada.** Ele
confirma que a palavra está escrita, não que o caminho funciona. Quem previne defeito antes de
alguém tropeçar nele é a execução.

Esta regra **substitui** o critério que estava escrito em `vssh-sso/docs/qualidade.md`, que
permitia guarda de regex quando ela "provasse ausência" — a primeira espécie da tabela acima. Aquela
seção foi reescrita dizendo que estava errada.

**Não há revisão retroativa**: as guardas antigas ficam onde estão, e cada uma sai quando o arquivo
dela for mexido por outro motivo.

**O que ainda sobra para leitura de fonte**, e é estreito: quando **não há execução possível** — a
junção entre dois arquivos que precisam concordar sobre um nome e nunca se encontram em runtime.
Mesmo aí, se der para rodar os dois lados, rode; e diga no arquivo por que não deu.
