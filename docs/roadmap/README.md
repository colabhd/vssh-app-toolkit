...
| 9 | [Editor do ambiente](08-editor-do-ambiente.md) — **passo 0: o endereço de um vssh-app deixa de ser uma porta** | toolkit + vssh-sso | ✅ **o contrato entregue** (0b) · 📋 **o caminho do portal** (0c/0d) · `backend.transport` no schema com padrão `socket`, `escutar()` só em socket unix, o portão do transporte no `vssh-app-run`, e os cinco manifestos declarando — quatro em socket, o xpra em `tcp` com prazo. Toolkit **4.0.0** (tag `v4`), `build-info.json` **4.1.0**. **Duas peças que eu ia construir já existiam** — o campo `minShellVersion` e o portão `podeInstalar()` —, e cheguei a criar um duplicado antes de achá-las. Falta o caminho do portal: healthcheck por `--unix-socket`, túnel, `socketPath` no proxy e no upgrade — e só então os onze lugares e o teto de 254 servidores morrem |
