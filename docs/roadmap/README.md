...
| 10 | [Motor X11](09-motor-x11.md) — **a primeira sessão real depois do item 4**: dois defeitos do 3a | `vsshapp-xpra` | ✅ **corrigido** (**0.6.2**) · os dois vêm da decisão central do 3a — **a janela X11 mudou de casa**, do `#screen` para o `body`, e tudo o que dependia disso parou de valer calado. (1) `set_cursor` chamava `.css()` num elemento do DOM: era `jQuery('#'+wid)` antes do 3a, virou `this.div`, e os dois `.css()` ficaram — `TypeError` no primeiro cursor da sessão. **A guarda nº 7 passou VERDE com isso**, porque procurava o SÍMBOLO `# Roadmap do ecossistema vssh-app
