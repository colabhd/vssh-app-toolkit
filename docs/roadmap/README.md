...
| 10 | [Motor X11](09-motor-x11.md) — item 8a: **os lugares da barra lateral são um palpite** | vssh-sso | 📋 **medido junto e deixado de fora de propósito** · a barra declara sete lugares com nome fixo e monta o caminho como `home + nome`. A lista **não bate com locale nenhum**: em `C` o XDG cria `Desktop`/`Documents`/`Music`, em `pt_BR` cria `Área de Trabalho`/`Documentos`/`Música` — seis dos sete atalhos apontam para pastas que podem não existir, e a barra os desenha igual. O conserto é o mesmo movimento do item 8: perguntar ao XDG em vez de adivinhar, com uma rota que devolva tudo numa ida de SSH |
