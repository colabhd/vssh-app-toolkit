...
//
// Por que isto existe: um vssh-app poderia fazer `server.listen(PORT, '127.0.0.1')`, e essa
...
function enderecoDoAmbiente(env = process.env) {
  ...
  // é velho demais, e dizer isso pelo nome poupa quem for depurar de procurar o defeito no app.
  if ((env.VSSH_APP_PORT || '').trim()) {
    const err = new Error(
      'Veio VSSH_APP_PORT, mas não VSSH_APP_SOCKET: o `vssh-app-run` deste servidor é antigo ' +
      'demais. O endereço de um app é um socket unix, e esta lib não binda porta. Atualize o ' +
      'provisionamento do servidor — o `minShellVersion` do manifesto existe para o PORTAL recusar ' +
      'esta combinação antes de ela chegar aqui.'
    );
