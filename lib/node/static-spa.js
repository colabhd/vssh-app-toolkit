...
//
// ── Mounts ───────────────────────────────────────────────────────────────────────────────────
// Um prefixo servido de OUTRO diretório, fora da raiz do bundle. Existe para as libs de navegador
// do toolkit, que desde a v4 vêm de `node_modules/vssh-app-toolkit/lib/web` — fora da raiz por
// construção, porque quem as instala é o npm.
//
// Antes disso cada app resolvia sozinho, e o custo aparecia inteiro: o Logseq mantinha uma rota à
// mão (`WEB_SHIM_ROUTES`, `sendVendorShim`) que servia os shims sem 304, sem confinamento e **sem
// o carimbo de versão** — justamente o arquivo cujo cache velho é o caso real narrado em "Carimbo
// de versão" mais abaixo. Um mount é o mesmo código do bundle apontando para outro diretório:
// mesmo confinamento por `realpath`, mesmo 304, mesmo `?v=`.
//
// **Precedência declarada: caminho direto > mount > alias.** O direto vence porque o app é dono
// da própria raiz; o mount vence o alias porque é uma afirmação sobre um prefixo, e o alias é um
// palpite de último recurso.
...
// A folha injetada é `<link>`, e não `<style>` com o conteúdo embutido: o navegador cacheia o
// arquivo (com o carimbo, para sempre), e o index continua `no-store` e pequeno.
//
// ⚠ **Elas saem ANTES dos `<script>`.** Não é arrumação: o `<link>` bloqueia a primeira pintura, e
// descobri-lo cedo é o que evita a página aparecer sem estilo por um quadro — fundo branco dentro
// de uma janela escura, que é o artefato que mais denuncia "isto é uma página web". Injetar por
// JavaScript teria o mesmo defeito por construção: a folha só seria descoberta quando o script
// rodasse.
function linkTags(sources, carimbo) {
  return sources.map((href) => {
    const v = carimbo(href);
    const url = v ? `${href}${String(href).includes('?') ? '&' : '?'}v=${v}` : String(href);
 ...
 * @param {string[]} [options.injectStyles] folhas injetadas como `<link rel="stylesheet">` antes de
 *   </head>, e **antes** dos `injectScripts`. Mesmo carimbo, mesma precedência de resolução. O caso
 *   canônico é a biblioteca de UI:
 *   `injectStyles: require('vssh-app-toolkit/web').ESTILOS.map((f) => `_vssh/${f}`)`.
 *   Um `<link>` escrito à mão no HTML do app funciona e é caminho suportado — só perde o carimbo,
 *   e para CSS o sintoma de bytes velhos é pior que para script: uma folha de cache antiga parece
 *   **defeito de design**, e ninguém abre o DevTools por causa de uma cor errada.
 ...
 * @param {Record<string,string>} [options.mounts] prefixo de URL (com `/` nas duas pontas) →
 *   diretório fora da raiz. Ver "Mounts" no topo. O caso canônico:
 *   `mounts: { '/_vssh/': require('vssh-app-toolkit/web').WEB_DIR }`
...
function createStaticSpa(options) {
  ...
  const injectStyles = options.injectStyles || [];
  const aliasPrefixes = Object.entries(options.aliasPrefixes || {});

  // Cada mount é canonicalizado do mesmo jeito que a raiz, e pela mesma razão: o confinamento
  // compara caminho real com caminho real, e duas grafias do mesmo diretório nunca casam. Um
  // mount cujo diretório ainda não existe segue lexical — é o caso de subir o backend antes do
  // `npm ci`, e a resposta certa ali é 404 no prefixo, não o processo morrer no boot.
  const mounts = Object.entries(options.mounts || {}).map(([prefix, dir]) => {
    if (!prefix.startsWith('/') || !prefix.endsWith('/')) {
      throw new TypeError(
        `mounts: o prefixo '${prefix}' precisa começar e terminar com '/' — sem isso, '/_vsshX' casaria com '/_vssh'.`);
    }
    let base = path.resolve(dir);
    try { base = fs.realpathSync.native(base); } catch { /* ainda não existe: segue lexical */ }
    return { prefix, base };
  });
  ...
  /** O arquivo em disco de um `src` injetado — pela MESMA precedência que serve o request. */
  function arquivoDoSrc(src) {
    ...
    for (const { prefix, base } of mounts) {
      if (pathname.startsWith(prefix)) return path.join(base, pathname.slice(prefix.length));
    }
    return path.join(root, pathname.replace(/^\/+/, ''));
  }
  const carimboDoSrc = (src) => hashDe(arquivoDoSrc(src));
  ...
  async function loadIndex() {
    ...
    // index mude é o caso normal (um `npm ci` mexe no `node_modules`, nunca no `index.html`).
    ...
    // cobrir.
    //
    // ⚠ As FOLHAS entram nesta chave junto com os scripts. Esquecê-las reproduziria o mesmo defeito
    // que o parágrafo acima descreve, e no caso delas o sintoma é pior: uma cor velha não parece
    // cache, parece decisão de design.
    const carimbos = [...injectStyles, ...injectScripts]
      .map((s) => `${s}=${carimboDoSrc(s) || ''}`).join('|');
    if (indexCache && indexCache.mtimeMs === stat.mtimeMs && indexCache.carimbos === carimbos) {
      return indexCache.body;
    }

    let html = await fsp.readFile(indexPath, 'utf8');
    if (injectStyles.length || injectScripts.length) {
      const tags = [
        injectStyles.length ? linkTags(injectStyles, carimboDoSrc) : '',
        injectScripts.length ? scriptTags(injectScripts, carimboDoSrc) : '',
      ].filter(Boolean).join('\n');
  ...
  // `base` é parâmetro porque um mount é a MESMA pergunta contra outro diretório. Uma segunda
  // função para isso seria uma segunda noção de "está confinado?", e é assim que uma delas fica
  // para trás quando a outra ganha uma defesa.
  async function statWithin(pathname, base = root) {
    const target = path.join(base, pathname);
  ...
  async function resolveTarget(pathname) {
    ...
    for (const { prefix, base } of mounts) {
      ...
      const montado = await statWithin(pathname.slice(prefix.length - 1), base);
