...
test('só VSSH_APP_PORT não é "faltou variável" — é servidor velho, e o erro diz isso', () => {
  ...
  // rodando à mão, o outro é um `vssh-app-run` anterior à Onda 9 num servidor de verdade. Um erro
  // que não distingue os dois manda quem depura procurar no app o que está no provisionamento.
  assert.throws(() => enderecoDoAmbiente({ VSSH_APP_PORT: '45999' }), (err) => {
    assert.equal(err.code, 'VSSH_APP_SERVIDOR_ANTIGO');
    assert.match(err.message, /vssh-app-run.*anterior|anterior.*vssh-app-run/);
