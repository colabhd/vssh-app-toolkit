...
test('sem VSSH_APP_SOCKET nem VSSH_APP_PORT, falha alto e nomeia as duas', () => {
  assert.throws(() => enderecoDoAmbiente({}), (err) => {
    assert.match(err.message, /VSSH_APP_SOCKET/);
    assert.match(err.message, /VSSH_APP_PORT/);
    return true;
  });
});

test('o socket tem precedência sobre a porta quando as duas vêm', () => {
  const alvo = enderecoDoAmbiente({ VSSH_APP_SOCKET: '/tmp/x.sock', VSSH_APP_PORT: '45999' });
  assert.equal(alvo.transporte, 'socket');
  assert.equal(alvo.caminho, '/tmp/x.sock');
});

test('porta inválida não passa por porta', () => {
  // '' e 'abc' viram NaN; 0 e 70000 estão fora da faixa. Todos têm de cair no erro que nomeia as
  // duas variáveis, e não num listen(NaN) que falha longe daqui.
  for (const porta of ['', 'abc', '0', '70000', '-1']) {
    assert.throws(() => enderecoDoAmbiente({ VSSH_APP_PORT: porta }), /VSSH_APP_SOCKET/);
  }
});

test('a porta, quando é o que veio', () => {
  assert.deepEqual(enderecoDoAmbiente({ VSSH_APP_PORT: '45999' }), { transporte: 'tcp', porta: 45999 });
});
...
test('o ramo TCP anuncia que o servidor está desatualizado, e cala quando é deliberado', async () => {
  // O aviso é o que impede este ramo de virar legado silencioso — e o silêncio com `tcpEsperado`
  // é o que impede o aviso de virar ruído para quem escolheu TCP de propósito no manifesto.
  const escrito = [];
  const original = process.stderr.write.bind(process.stderr);
  process.stderr.write = (chunk, ...resto) => { escrito.push(String(chunk)); return original(chunk, ...resto); };

  const porta = async () => new Promise((ok) => {
  ...
  try {
    await escutar(a, { env: { VSSH_APP_PORT: String(await porta()) } });
    assert.ok(escrito.some((l) => /vssh-app-listen.*VSSH_APP_PORT/.test(l)),
      'o ramo TCP subiu sem dizer que este servidor está atrás da Onda 9');

    escrito.length = 0;
    await escutar(b, { env: { VSSH_APP_PORT: String(await porta()) }, tcpEsperado: true });
    assert.equal(escrito.length, 0, 'avisou mesmo com tcpEsperado — vira ruído para quem escolheu');
  } finally {
    process.stderr.write = original;
    await Promise.all([a, b].map((s) => new Promise((ok) => { try { s.close(ok); } catch { ok(); } })));
  }
});

test('escuta em TCP quando é a porta que vem', async () => {
  const srv = http.createServer((_q, r) => { r.writeHead(200); r.end('ok'); });
  try {
    // Porta 0 não serve aqui (o helper a recusa, e com razão), então pegamos uma livre de verdade.
    const livre = await new Promise((ok) => {
      const s = net.createServer();
      s.listen(0, '127.0.0.1', () => { const p = s.address().port; s.close(() => ok(p)); });
    });
    const r = await escutar(srv, { env: { VSSH_APP_PORT: String(livre) } });
    assert.equal(r.transporte, 'tcp');
    assert.equal(r.endereco, `127.0.0.1:${livre}`);
  } finally {
    await new Promise((ok) => srv.close(ok));
  }
});
