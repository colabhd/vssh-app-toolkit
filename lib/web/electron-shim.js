...
//   - clipboard de IMAGEM: `vssh.clipboard` cobre ARQUIVOS e imagem, mas não o texto do X11;
...
(function () {
  ...
  // ── shell ─────────────────────────────────────────────────────────────────
  const shell = {
    // ⚠ Isto era `window.open(url, '_blank')`, e a tradução estava errada desde que `vssh.openUrl`
    // existe: no Electron, `openExternal` abre no navegador DO SISTEMA — o de fora do app. Aqui o
    // "de fora do app" é o navegador do AMBIENTE, não uma aba do navegador que hospeda o ambiente
    // inteiro. E a diferença não é de enquadramento: o navegador do ambiente resolve a rede a
    // partir do servidor Linux, então um `http://localhost:3000` do app chega ao loopback certo.
    //
    // A promessa segue a do Electron e **rejeita** quando o ambiente recusa. Engolir aqui deixaria
    // um `openExternal` que não abre nada e não diz nada.
    openExternal(url) { return window.vssh.openUrl(url).then(() => undefined); },
