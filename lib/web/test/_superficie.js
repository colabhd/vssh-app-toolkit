...
/**
 * O `vssh` publicado pelo shim, para quem precisa de um VALOR e não da árvore de nomes.
 *
 * `tests/lib-version.test.js` o usa para ler o `libVersion` executando, em vez de recortar o
 * literal do fonte: três nomes diferentes podem apontar para o mesmo literal, e só a leitura em
 * runtime diz qual deles o app recebe.
 */
function vsshCarregado() {
  return _carregar({ addEventListener() {}, removeEventListener() {},
                     parent: { postMessage() {} }, matchMedia: () => ({ matches: false }) });
}

module.exports = { superficieReal, caminhosDaSuperficie, vsshCarregado };
