...
// Os testes do `tauri-shim`. Ele nasceu sem nenhum, e os primeiros acharam três defeitos.
//
...
test('fs.exists delega — e "não pude perguntar" NÃO vira "não existe"', async () => {
  const presente = tauri({ respostas: { 'fs.exists': true } });
  assert.equal(await presente.api.fs.exists('/a/x'), true);
  igual(presente.nomes(), ['fs.exists'], 'ainda está perguntando pelo stat');

  const ausente = tauri({ respostas: { 'fs.exists': false } });
  assert.equal(await ausente.api.fs.exists('/a/x'), false);

  // Isto era `stat(p).catch(() => false)`: permissão negada e rede fora saíam como "não existe",
  // e o app criava por cima de um arquivo que estava lá. O erro tem de subir.
  const quebrado = tauri({ respostas: { 'fs.exists': Promise.reject(new Error('403 sem permissão')) } });
  await assert.rejects(() => quebrado.api.fs.exists('/a/x'), /403/);
});

test('fs.rename e fs.copyFile chegam às ops novas da ponte', async () => {
  const { api, chamadas } = tauri();
  await api.fs.rename('/a/x.md', '/a/y.md');
  await api.fs.copyFile('/a/y.md', '/b/y.md');
  igual(chamadas, [
    ['fs.rename', '/a/x.md', '/a/y.md'],
    ['fs.copy', '/a/y.md', '/b/y.md'],
  ]);
});
...
test('shell.open separa URL de caminho do servidor', async () => {
  // As duas metades vão para o AMBIENTE: a URL para o navegador dele, o caminho para o
  // visualizador. Nenhuma das duas vira aba do navegador hospedeiro.
  const { api, chamadas, janelasAbertas } = tauri();
  await api.shell.open('https://exemplo.org/doc');
  await api.shell.open('/casa/relatorio.pdf');
  igual(chamadas, [['openUrl', 'https://exemplo.org/doc'], ['openFile', '/casa/relatorio.pdf']]);
  igual(janelasAbertas, [], 'nenhuma aba do hospedeiro');
});
