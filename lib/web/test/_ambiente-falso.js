...
function carregar(arquivo, opcoes = {}) {
  ...
  const vssh = {
    ...
    openUrl(u)    { grava('openUrl', u); return Promise.resolve(resposta('openUrl', null)); },
    ...
    fs: {
      ...
      exists(p)        { grava('fs.exists', p); return Promise.resolve(resposta('fs.exists', true)); },
      rename(a, b, o)  { grava('fs.rename', a, b, ...(o ? [o] : [])); return Promise.resolve(resposta('fs.rename', { ok: true })); },
      copy(a, b, o)    { grava('fs.copy', a, b, ...(o ? [o] : [])); return Promise.resolve(resposta('fs.copy', { ok: true })); },
    },
