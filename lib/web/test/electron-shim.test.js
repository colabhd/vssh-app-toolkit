...
// Os testes do `electron-shim`. Ele nasceu sem nenhum, e os primeiros acharam três defeitos.
//
...
test('openExternal vai para o navegador do ambiente, e não para uma aba', async () => {
  // No Electron, `openExternal` é "abre fora do meu app". Aqui, "fora do app" é o navegador do
  // AMBIENTE — uma aba do navegador hospedeiro tiraria a pessoa do ambiente inteiro, e resolveria
  // a rede da máquina de quem lê em vez da do servidor.
  const { api, chamadas, janelasAbertas } = electron();
  await api.shell.openExternal('https://exemplo.org');
  igual(chamadas, [['openUrl', 'https://exemplo.org']]);
