...
(function () {
  ...
  const vssh = {
    ...
    window: {
       ...
       *
       * Chame no `pointerdown` da sua barra de título e não faça mais nada: o arraste inteiro é
       * do ambiente daí em diante, com o mesmo limiar, o mesmo containment e o mesmo
       * encaixe de janelas que qualquer outra. Não mande `pointermove`.
       *
       * As coordenadas são as do evento (`e.clientX`, `e.clientY`), como você as recebeu. Quem
       * traduz para a tela é quem conhece o seu quadro — você não precisa saber onde está.
       *
       * O que isto SUBSTITUI: desenhar um retângulo invisível sobre a sua barra e manter a
       * geometria dele em dia. Não há geometria aqui.
       */
      arrastar(x, y) { return post('window', { op: 'drag-start', x, y }); },
