...
(function () {
  ...
  //
  // O desktop VSSH tem um mixer de volume na barra, e ele é o mixer DE VERDADE: um
  ...
  const vssh = {
    ...
    media: {
      ...
      /**
       * O que está tocando: `{ titulo, subtitulo, capa }`. Chame a cada troca de faixa.
       *
       * ⚠ **Sem isto o ambiente ADIVINHA, e a adivinhação erra exatamente onde mais importa.** O
       * shell alcança o `<video>` do app e tira o nome da URL da fonte — o que funciona para um
       * arquivo (`?caminho=/home/ana/Vídeos/aula.mkv`) e falha para tudo que é montado por MSE:
       * um `blob:https://host/aefc5fdf-64ee-46f9-95b7-cd2e863e0cce` não tem nome, e o que aparecia
       * na central de mídia era o UUID.
       *
       * `capa` é uma URL de imagem — relativa ao app, como as outras. Sem ela o ambiente desenha
       * o ícone genérico de vídeo ou áudio, que é o que sempre fez.
       *
       * Passar `null` (ou nada) devolve a decisão ao ambiente.
       */
      agora(o) {
        const d = o || {};
        post('media-agora', {
          titulo: d.titulo == null ? '' : String(d.titulo).slice(0, 300),
          subtitulo: d.subtitulo == null ? '' : String(d.subtitulo).slice(0, 300),
          capa: d.capa == null ? '' : String(d.capa).slice(0, 2000),
        });
      },
    },
  ...
  // documentação dizendo por escrito que aquele gancho ficava. Aqui ela viaja com o app: a cópia vai no
