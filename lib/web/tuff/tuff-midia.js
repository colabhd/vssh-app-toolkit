...
(function () {
   ...
   *
   * ⚠ **É a opção que separa esta peça de "só serve para arquivo local".** Um vídeo que o servidor
   * está remuxando ou mesclando chega por um cano: sem `Content-Length` não há `Accept-Ranges`, e
   * sem `Accept-Ranges` o `<video>` relata `duration: Infinity` e recusa `currentTime =`. Com a
   * régua do próprio elemento, a trilha fica sem tamanho e o arraste não faz nada — e o app não tem
   * como consertar por fora, porque quem lê `video.duration` é esta função.
   *
   ...
   * ```
   *
   * **Sem a opção, o comportamento é o de antes, byte a byte** — os valores continuam vindo crus do
   * elemento, inclusive o `NaN` que vira `--:--` antes dos metadados. Cada função é opcional por si:
   * dar só `duracao` já resolve o caso comum de o servidor conhecer o tamanho e o cano não.
   *
   * O buffer é o único que precisa de tradução, e ela sai de graça: `video.buffered` fala nas
   * coordenadas do CANO, e a diferença entre `atual()` e `video.currentTime` é justamente o
   * deslocamento entre as duas réguas. Sem isso, a barra de carregado de um vídeo buscado aos 30
   * min apareceria no começo da linha.
   *
   * Devolve `{ destruir() }`.
   */
  function player(raiz, video, opcoes) {
    ...
    // da trilha e o total à direita — é onde todo player de desktop os põe, e juntá-los no meio
    // desperdiça a largura que a trilha usaria. Sem estes dois, um app com transporte tinha de
    // reescrever o relógio à mão e perdia a fonte de tempo junto.
    ...
    function pintar() {
      ...
      //
      // A BUSCA é nas coordenadas do elemento — é o que `video.buffered` fala. O resultado é levado
      // para as da linha do tempo pelo deslocamento entre as duas réguas, que é exatamente a
      // diferença entre as duas maneiras de perguntar onde estamos.
      let ate = null;
      ...
      // foi pedido. Sem fonte de tempo o deslocamento é zero e isto é o de antes.
    ...
    if (btPlay) {
      ...
      // qualquer pintura. Isto era `btPlay.textContent = '▶'` incondicional — e `textContent`
      // apaga os filhos: um `<button><svg><use href="#ico-play"></svg></button>`, que é exatamente
      // o que `TuffIcones` (deste mesmo pacote) manda escrever, perdia o ícone no primeiro quadro e
      // virava um caractere solto no meio do transporte. A biblioteca desfazia o que a biblioteca
      // pedia.
      //
      // O `aria-label` continua sendo escrito sempre: ele é o nome do botão para quem não vê o
      // glifo nem o ícone, e é a metade que nenhum app deve ter de repetir.
