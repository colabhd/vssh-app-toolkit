...
(function () {
  ...
  // ── shell ─────────────────────────────────────────────────────────────────
  const shell = {
    // Um path do servidor abre no visualizador do desktop; uma URL abre no navegador DO AMBIENTE.
    //
    // ⚠ A segunda metade era `window.open(target, '_blank')` e estava errada desde que
    // `vssh.openUrl` existe — ver o mesmo trecho no `electron-shim`. Uma aba do navegador
    // hospedeiro tira a pessoa do ambiente e resolve a rede da máquina errada.
    open(target) {
      if (/^https?:\/\//i.test(target)) return window.vssh.openUrl(target).then(() => undefined);
